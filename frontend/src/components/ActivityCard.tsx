import React from 'react';
import { Translations } from '../services/i18n';

interface ActivityCardProps {
  monthlyCount?: number;
  monthlyHours?: number;
  currentStreak?: number;
  t: Translations;
}

export const ActivityCard: React.FC<ActivityCardProps> = ({
  monthlyCount = 0,
  monthlyHours = 0,
  currentStreak = 0,
  t,
}) => {
  const hasData = monthlyCount > 0 || monthlyHours > 0 || currentStreak > 0;

  return (
    <div className="glass-card p-4 rounded-2xl space-y-3">
      <div className="text-xs text-gray-400 font-medium">{t.activity.this_month}</div>

      <div className="grid grid-cols-3 gap-2 text-center">
        <div>
          <div className="text-lg font-bold text-white">{monthlyCount}</div>
          <div className="text-[10px] text-gray-400">{t.activity.added}</div>
        </div>
        <div>
          <div className="text-lg font-bold text-white">
            {monthlyHours}
            {t.activity.hours_suffix}
          </div>
          <div className="text-[10px] text-gray-400">{t.activity.spent}</div>
        </div>
        <div>
          <div className="text-lg font-bold text-white">{currentStreak}</div>
          <div className="text-[10px] text-gray-400">{t.activity.streak}</div>
        </div>
      </div>

      {/* Sparkline Wave Chart / Zero Baseline */}
      <div className="pt-2">
        {hasData ? (
          <svg className="w-full h-12 overflow-visible" viewBox="0 0 300 40">
            <defs>
              <linearGradient id="chartGradient" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor="#6C5CE7" stopOpacity="0.4" />
                <stop offset="100%" stopColor="#6C5CE7" stopOpacity="0.0" />
              </linearGradient>
            </defs>
            <path d="M 0,30 Q 30,10 60,25 T 120,15 T 180,28 T 240,10 T 300,20 L 300,40 L 0,40 Z" fill="url(#chartGradient)"></path>
            <path d="M 0,30 Q 30,10 60,25 T 120,15 T 180,28 T 240,10 T 300,20" fill="none" stroke="#6C5CE7" strokeWidth="2.5" strokeLinecap="round"></path>
            <circle cx="60" cy="25" r="3" fill="#6C5CE7"></circle>
            <circle cx="120" cy="15" r="3" fill="#6C5CE7"></circle>
            <circle cx="180" cy="28" r="3" fill="#6C5CE7"></circle>
            <circle cx="240" cy="10" r="4" fill="#00CEC9" stroke="#ffffff" strokeWidth="1.5"></circle>
          </svg>
        ) : (
          <svg className="w-full h-8 overflow-visible" viewBox="0 0 300 20">
            <line x1="0" y1="15" x2="300" y2="15" stroke="#232734" strokeWidth="2" strokeDasharray="4 4" />
          </svg>
        )}
      </div>
    </div>
  );
};
