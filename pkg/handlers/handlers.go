...
func (h *Handler) GetListRecommendations(w http.ResponseWriter, r *http.Request) {
	...
	prompt := fmt.Sprintf(`Ты — эксперт в подборе фильмов, сериалов, книг и игр. Твоя задача: проанализировать контекст пользователя и сгенерировать 20 новых рекомендаций.
Тема списка: "%s"
Категория: %s
Годы: %s
Страны: %s
...
Сгенерируй 20 рекомендаций, которые идеально подходят по духу, смыслу, категории (%s), эпохе и жанру к "Теме списка" и похожи на элементы из Текущего списка пользователя.
