...
func (h *Handler) GetListRecommendations(w http.ResponseWriter, r *http.Request) {
	...
	prompt := fmt.Sprintf(`Ты — эксперт в подборе фильмов, сериалов, книг и игр. Твоя задача: проанализировать контекст пользователя и сгенерировать ровно 20 новых рекомендаций.
Тема списка: "%s"
Категория: %s
Годы: %s
Страны: %s
...
Сгенерируй РОВНО 20 рекомендаций (не больше и не меньше), которые идеально подходят по духу, смыслу, категории (%s), эпохе и жанру к "Теме списка" и похожи на элементы из Текущего списка пользователя.
...
3. Разнообразие: все 20 элементов должны быть самостоятельными произведениями из разных франшиз (без сиквелов, приквелов и спин-оффов).
4. Категорически ЗАПРЕЩЕНО указывать произведения, которые уже присутствуют во входящем списке пользователя.
5. Формат ответа: только сырой JSON-массив из 20 строк с официальными русскими названиями. Пример: ["Название 1", "Название 2", ...]. 
6. Без markdown-разметки и без сопроводительного текста.
7. ВАЖНО: Список должен состоять строго из 20 позиций. Выдай ровно 20 названий.`,
